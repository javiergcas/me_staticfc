...
do
    ...
    afni_proc.py                                                                                             \
                ...
                -dsets_me_run ${PRCSDATA_DIR}/${SBJ}/D03_NORDIC/${SBJ}_${SES}_task-rest_echo-1_bold.NORDIC.nii.gz  \
                              ${PRCSDATA_DIR}/${SBJ}/D03_NORDIC/${SBJ}_${SES}_task-rest_echo-2_bold.NORDIC.nii.gz  \
                              ${PRCSDATA_DIR}/${SBJ}/D03_NORDIC/${SBJ}_${SES}_task-rest_echo-3_bold.NORDIC.nii.gz  \
                -echo_times ${ECHOTIMES}                                                                     \
                -combine_method m_tedana                                                                     \
                -combine_opts_tedana "--mask ${PRJDIR}/prcs_data/${SBJ}/D02_Preproc_fMRI_${SES}/mask_group+tlrc.HEAD --verbose" \
                ...
                -out_dir ${OUT_DIR}                                                                          \
                -script ${AFNI_PROC_OUT_DIR}/S04_Afni_Preproc_fMRI.${SBJ}_${SES}.NORDIC.sh                   \
                -regress_compute_tsnr yes                                                                    \
                -regress_make_cbucket yes                                                                    \
                -scr_overwrite

       # Add line for this subject to the Swarm file
       echo "module load afni; source /data/SFIMJGC_HCP7T/Apps/miniconda38/etc/profile.d/conda.sh && conda activate tedana_2024a; tcsh -xef ${AFNI_PROC_OUT_DIR}/S04_Afni_Preproc_fMRI.${SBJ}_${SES}.NORDIC.sh 2>&1 | tee ${AFNI_PROC_OUT_DIR}/output.S04_Afni_Preproc_fMRI.${SBJ}_${SES}.NORDIC.txt" >> ${SWARM_PATH}
done
