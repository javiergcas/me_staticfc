...
do
    ...
    already_done=`grep ${AFNI_PROC_OUT_DIR}/S04_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}_NORDIC-on.sh ${SWARM_PATH} | wc -l`
    if [ ${already_done} == "1" ]; then
       echo "${AFNI_PROC_OUT_DIR}/S04_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}_NORDIC-on.sh already exits --> skipping."
    else

    if [ ! -f ${ANAT_PROC_DIR}/anatQQ.${SBJ}.nii.gz ]; then
       echo "${SBJ} ${SES}" >> ${RESOURCES_DIR}/S04_NotAvailable.txt
    else
