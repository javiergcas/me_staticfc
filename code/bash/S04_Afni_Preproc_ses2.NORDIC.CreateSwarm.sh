...
do
    ...
    already_done=`grep ${AFNI_PROC_OUT_DIR}/S02_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}.sh ${SWARM_PATH} | wc -l`
    if [ ${already_done} == "1" ]; then
       echo "${AFNI_PROC_OUT_DIR}/S02_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}.sh already exits --> skipping."
    else
    ...
    if [ ! -f ${ANAT_PROC_DIR}/anatQQ.${SBJ}.nii.gz ]; then
       ...
       afni_proc.py                                                                                          \
                ...
                -out_dir ${OUT_DIR}                                                                          \
                -script ${AFNI_PROC_OUT_DIR}/S02_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}.sh                          \
                -regress_compute_tsnr yes                                                                    \
                -regress_make_cbucket yes                                                                    \
                -scr_overwrite

       # Add line for this subject to the Swarm file
       echo "module load afni; source /data/SFIMJGC_HCP7T/Apps/miniconda38/etc/profile.d/conda.sh && conda activate tedana_2024a; tcsh -xef ${AFNI_PROC_OUT_DIR}/S02_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}.sh 2>&1 | tee ${AFNI_PROC_OUT_DIR}/output.S02_Afni_Preproc_fMRI_NORDIC.${SBJ}_${SES}.txt" >> ${SWARM_PATH}
    fi
fi
done
