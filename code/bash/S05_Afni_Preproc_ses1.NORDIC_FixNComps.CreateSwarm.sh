...
AFNI_PROC_OUT_DIR=`echo ${SCRIPTS_DIR}/S05_Afni_Preproc_${SES}.NORDIC_FixNComps`
RESOURCES_DIR='/data/SFIMJGC_HCP7T/BCBL2024/resources/'
USERNAME=`whoami`
SWARM_PATH=`echo ${PRJDIR}/swarm.${USERNAME}/S05_Afni_Preproc_${SES}.NORDIC_FixNComps.SWARM.sh`
LOGS_DIR=`echo ${PRJDIR}/logs.${USERNAME}/S05_Afni_Preproc_${SES}.NORDIC_FixNComps.logs`
...
do
    ...
    #already_done=`grep ${SWARM_PATH} ${AFNI_PROC_OUT_DIR}/S05_Afni_Preproc_fMRI_NORDIC_FixNComps.${SBJ}_${SES}.sh | wc -l`
		#if [ ${already_done} == "1" ]; then
    #   echo "${AFNI_PROC_OUT_DIR}/S05_Afni_Preproc_fMRI_NORDIC_FixNComps.${SBJ}_${SES}.sh already exits --> skipping."
    #else
    ...
    if [ ! -f ${ANAT_PROC_DIR}/anatQQ.${SBJ}.nii.gz ]; then
       ...
       OUT_DIR=`echo ${PRJDIR}/prcs_data/${SBJ}/D05_Preproc_fMRI_${SES}_NORDIC_FixNComps`
       ...
       afni_proc.py                                                                                          \
                ...
                -dsets_me_run ${PRJDIR}/prcs_data/${SBJ}/D03_NORDIC/${SBJ}_${SES}_task-rest_echo-1_bold.NORDIC.nii.gz  \
                              ${PRJDIR}/prcs_data/${SBJ}/D03_NORDIC/${SBJ}_${SES}_task-rest_echo-2_bold.NORDIC.nii.gz  \
                ...
                -combine_opts_tedana "--tedpca 88 --verbose"                                                 \
                ...
                -out_dir ${OUT_DIR}                                                                          \
                -script ${AFNI_PROC_OUT_DIR}/S05_Afni_Preproc_fMRI_NORDIC_FixNComps.${SBJ}_${SES}.sh         \
                -regress_compute_tsnr yes                                                                    \
                -regress_make_cbucket yes                                                                    \
                -scr_overwrite

       # Add line for this subject to the Swarm file
       echo "module load afni; source /data/SFIMJGC_HCP7T/Apps/miniconda38/etc/profile.d/conda.sh && conda activate tedana_2024a; tcsh -xef ${AFNI_PROC_OUT_DIR}/S05_Afni_Preproc_fMRI_NORDIC_FixNComps.${SBJ}_${SES}.sh 2>&1 | tee ${AFNI_PROC_OUT_DIR}/output.S05_Afni_Preproc_fMRI_NORDIC_FixNComps.${SBJ}_${SES}.txt" >> ${SWARM_PATH}
    fi
#fi
done
