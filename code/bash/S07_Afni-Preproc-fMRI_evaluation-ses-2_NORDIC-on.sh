...
AFNI_PROC_OUT_DIR=`echo ${SCRIPTS_DIR}/S07_Afni-Preproc-fMRI_evaluation-${SES}_NORDIC-on`
USERNAME=`whoami`
SWARM_PATH=`echo ${PRJDIR}/swarm.${USERNAME}/S07_Afni-Preproc-fMRI_evaluation-${SES}_NORDIC-on.SWARM.sh`
LOGS_DIR=`echo ${PRJDIR}/logs.${USERNAME}/S07_Afni-Preproc-fMRI_evaluation-${SES}_NORDIC-on.logs`
...
do
    ...
    AFNI_PROC_OUT_SCRIPT=`echo ${AFNI_PROC_OUT_DIR}/S07_Afni-Preproc-fMRI_evaluation_${SBJ}-${SES}_NORDIC-on.sh`
    AFNI_PROC_OUT_LOGFILE=`echo ${AFNI_PROC_OUT_DIR}/output.S07_Afni-Preproc-fMRI_evaluation_${SBJ}-${SES}_NORDIC-on.txt`
    TEDANA_MASK_PATH=`echo ${PRJDIR}/prcs_data/${SBJ}/D03_Preproc_${SES}_NORDIC-off/mask_group+tlrc.HEAD`
    ...
    afni_proc.py                                                                                             \
                ...
                -dsets_me_run ${PRCSDATA_DIR}/${SBJ}/D02_NORDIC/${SBJ}_${SES}_task-rest_echo-1_bold.NORDIC.nii.gz \
                              ${PRCSDATA_DIR}/${SBJ}/D02_NORDIC/${SBJ}_${SES}_task-rest_echo-2_bold.NORDIC.nii.gz \
                              ${PRCSDATA_DIR}/${SBJ}/D02_NORDIC/${SBJ}_${SES}_task-rest_echo-3_bold.NORDIC.nii.gz \
                -echo_times ${ECHOTIMES}                                                                     \
                -combine_method m_tedana                                                                     \
                -combine_opts_tedana "--mask ${TEDANA_MASK_PATH} --verbose"                                  \
                ...
                -out_dir ${OUT_DIR}                                                                          \
                -script ${AFNI_PROC_OUT_SCRIPT}                                                              \
                -regress_compute_tsnr yes                                                                    \
                -regress_make_cbucket yes                                                                    \
                -scr_overwrite

       # Add line for this subject to the Swarm file
       echo "module load afni; source /data/SFIMJGC_HCP7T/Apps/miniconda38/etc/profile.d/conda.sh && conda activate tedana_2025a; tcsh -xef ${AFNI_PROC_OUT_SCRIPT} 2>&1 | tee ${AFNI_PROC_OUT_LOGFILE}; mv ${OUT_DIR}/tedana_r01 ${OUT_DIR}/tedana_fastica" >> ${SWARM_PATH}
done
