...
echo "++ Compute Mean of data post volreg"
echo "++ ================================"
...
echo "++ Compute GS for each echo separately post volreg"
echo "++ ==============================================="
for EC in e01 e02 e03
do
    3dROIstats -quiet \
...
# NOTE: Afni_proc computes CompCorr regressors based on OC data, which can be influenced by whether or not NORDIC is in place (e.g., S0 and t2s maps can differ)
...
echo "++ 2. Create GLM regression matrix per-echo for later use in 3dTproject..."
echo "++ ======================================================================="
...
echo "++ 3. Do regression for the Basic and GSR pipelines per-echo..."
echo "++ ============================================================"
...
# ----------------------
INTERP_MODE='ALL'  # we only extract timeseries for ALL scenario post-regression
echo "++ Extracting ROI Timeseries per echo for atlas [${ATLAS_NAME}] | Basic Denosing"
echo "================================================================================"
for EC in e01 e02 e03
do
  echo " + Extracting FC matrix for [${EC} + Basic Denoising]"
  ...
  echo " + Extracting ROI timeseries with 3dROIstats (more decimals than 3dNetCorr) [${EC} + Basic Denoising]"
  3dROIstats -quiet \
             -mask ${ATLAS_PATH} \
...
echo "++ Extracting ROI Timeseries per echo for atlas [${ATLAS_NAME}] | GSR Denosing"
echo "=============================================================================="
for EC in e01 e02 e03
do
  echo " + Extracting ROI Timeseries and connectivity for [${EC}, GSR]"
  3dNetCorr -overwrite                                                                            \
            -push_thru_many_zeros                                                                 \
            -mask ../D03_Preproc_${SES}_NORDIC-off/mask_tedana_at_least_one_echo.nii.gz           \
            -in_rois ${ATLAS_PATH}                                                                \
  ...
  echo " + Extracting ROI timeseries with 3dROIstats (more decimals than 3dNetCorr) [${EC} + GSR Denoising]"
  3dROIstats -quiet                                                                               \
             -mask ${ATLAS_PATH}                                                                  \
...
# We are also gonna have the ROI TS and FC prior to any regression (for comparison) | Not used in manuscript
echo "++ Extracting ROI Timeseries per echo for atlas [${ATLAS_NAME}] | No Regression"
echo "==============================================================================="
for EC in e01 e02 e03
do
...
for EC in e01 e02 e03
do
  for SCENARIO in ALL_Basic ALL_GS ALL_NoRegression
  do
    # Create a voxel-wise TSNR map for all voxels with good ME data
