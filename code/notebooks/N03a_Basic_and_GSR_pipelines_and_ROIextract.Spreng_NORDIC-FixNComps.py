...
# Also, let's check that all afni_proc jobs for the NORDIC data finished correctly.

for sbj,ses in tqdm(dataset_info_df.index):
    qc_path = osp.join(PRCS_DATA_DIR,sbj,f'D05_Preproc_fMRI_{ses}_NORDIC_FixNComps',f'QC_{sbj}','index.html')
    if not osp.exists(qc_path):
        print('++ WARNING: [%s] is missing --> likely some issue with afni_proc.'% qc_path)

# # 2. Create Swarm Script to Extract ROI TS from Basic denoised data

script_path = osp.join(PRJ_DIR,f'swarm.{username}',f'N03a_Basic_and_GSR_pipeline_and_ROIextract.{ATLAS_NAME}.NORDIC_FixNComps.SWARM.sh')
print(script_path)

log_path = osp.join(PRJ_DIR,f'logs.{username}',f'N03a_Basic_and_GSR_pipeline_and_ROIextract.{ATLAS_NAME}.NORDIC_FixNComps.log')
...
with open(script_path, 'w') as the_file:
    ...
    for sbj,ses in list(dataset_info_df.index):
        ...
        AFNIPROC_OUTDIR=osp.join(PRCS_DATA_DIR,sbj,f'D05_Preproc_fMRI_{ses}_NORDIC_FixNComps')
        the_file.write(f'export SBJ={sbj} SES={ses} ATLAS_NAME={ATLAS_NAME} ATLAS_PATH={atlas_path} ATLASES_DIR={ATLASES_DIR} AFNIPROC_OUTDIR={AFNIPROC_OUTDIR}; sh  {CODE_DIR}/bash/N03a_Basic_and_GSR_pipeline_and_ROIextract.sh \n')
the_file.close()     
...
#
# ```bash
# swarm -f /data/SFIMJGC_HCP7T/BCBL2024/swarm.javiergc/N03a_Basic_and_GSR_pipeline_and_ROIextract.Power264.NORDIC.SWARM.sh \
#       -g 16 -t 8 -b 10 --time 00:20:00 \
#       --logdir /data/SFIMJGC_HCP7T/BCBL2024/logs.javiergc/N03a_Basic_and_GSR_pipeline_and_ROIextract.Power264.NORDIC.log \
...
for sbj,ses in list(dataset_info_df.index):
    netcc_path = osp.join(PRCS_DATA_DIR,sbj,f'D05_Preproc_fMRI_{ses}_NORDIC_FixNComps',f'errts.{sbj}.r01.e02.volreg.scale.tproject_ALL_Basic.{ATLAS_NAME}_000.netcc')
    if not osp.exists(netcc_path):
        print('++ WARNING: %s is missing'% netcc_path)
    netcc_path = osp.join(PRCS_DATA_DIR,sbj,f'D05_Preproc_fMRI_{ses}_NORDIC_FixNComps',f'errts.{sbj}.r01.e02.volreg.scale.tproject_ALL_GSasis.{ATLAS_NAME}_000.netcc')
    if not osp.exists(netcc_path):
        print('++ WARNING: %s is missing'% netcc_path)
...
for sbj,ses in tqdm(list(dataset_info_df.index)):
    for scenario in ['ALL_Basic','ALL_GSasis','KILL_Basic','KILL_GSasis'] :
        netcc_path = osp.join(PRCS_DATA_DIR,sbj,f'D04_Preproc_fMRI_{ses}_NORDIC',f'errts.{sbj}.r01.e02.volreg.scale.tproject_{scenario}.{ATLAS_NAME}_000.netcc') 
