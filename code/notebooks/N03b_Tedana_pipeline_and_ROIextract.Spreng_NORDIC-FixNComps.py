...
script_path = osp.join(PRJ_DIR,f'swarm.{username}',f'N03b_Tedana_pipeline_and_ROIextract.{ATLAS_NAME}.NORDIC_FixNComps.SWARM.sh')
print(script_path)

log_path = osp.join(PRJ_DIR,f'logs.{username}',f'N03b_Tedana_pipeline_and_ROIextract.{ATLAS_NAME}.NORDIC_FixNComps.log')
...
with open(script_path, 'w') as the_file:
    ...
    for sbj,ses in list(dataset_info_df.index):
        ...
        the_file.write(f'export SBJ={sbj} SES={ses} ATLAS_NAME={ATLAS_NAME} ATLAS_PATH={atlas_path} ATLASES_DIR={ATLASES_DIR} AFNIPROC_OUTDIR={AFNIPROC_OUTDIR}; sh  {CODE_DIR}/bash/N03b_Tedana_pipeline_and_ROIextract.sh \n')
the_file.close()     
...
# swarm -f /data/SFIMJGC_HCP7T/BCBL2024/swarm.javiergc/N03b_Tedana_pipeline_and_ROIextract.Power264.NORDIC.SWARM.sh -g 16 -t 8 -b 10 --time 00:20:00 --logdir /data/SFIMJGC_HCP7T/BCBL2024/logs.javiergc/N03b_Tedana_pipeline_and_ROIextract.Power264.NORDIC.log --partition quick,norm --module afni
# ```
#
# ***
...
for sbj,ses in list(dataset_info_df.index):
    for scenario in ['ALL_Tedana','KILL_Tedana','ZERO_Tedana','NTRP_Tedana','ALL_TedanaGSasis','KILL_TedanaGSasis','ZERO_TedanaGSasis','NTRP_TedanaGSasis']:
        for e in ['e01','e02','e03']:
            netcc_path = osp.join(PRCS_DATA_DIR,sbj,f'D04_Preproc_fMRI_{ses}_NORDIC',f'errts.{sbj}.r01.{e}.volreg.scale.tproject_{scenario}.{ATLAS_NAME}_000.netts')
...
for sbj,ses in tqdm(list(dataset_info_df.index)):
    for pipeline,censor_mode,te in scenarios:
        netcc_path = osp.join(PRCS_DATA_DIR,sbj,f'D04_Preproc_fMRI_{ses}_NORDIC',f'errts.{sbj}.r01.{te}.volreg.scale.tproject_{censor_mode}_{pipeline}.{ATLAS_NAME}_000.netcc')
