...
#
# This notebook looks at item 2 using **Pearson's Correlation** as our estimate of functional connectivity. Our approach here follows that of Powers' et al. in ["Ridding fMRI dta of motion-related influences"](https://www.pnas.org/doi/10.1073/pnas.1720985115?url_ver=Z39.88-2003&rfr_id=ori:rid:crossref.org&rfr_dat=cr_pub%20%200pubmed)
#
#
# ***
...
for pipeline in ['Basic','GSasis','Tedana','TedanaGSasis']:
    for censor_mode in ['ALL','KILL']:
        fc_all_sbjs_R[f'across_echoes-{pipeline}-{censor_mode}']  = pd.DataFrame(np.ones((Nscans,Ncons))* np.nan, index=dataset_info_df.index)
        fc_all_sbjs_Z[f'across_echoes-{pipeline}-{censor_mode}']  = pd.DataFrame(np.ones((Nscans,Ncons))* np.nan, index=dataset_info_df.index)

        for i,(sbj,ses) in enumerate(tqdm(dataset_scan_list,desc='%s,%s' % (pipeline,censor_mode))):
            # Create empty DataArray to hold FC for all echo-pairs
            aux_fc_xr = xr.DataArray(dims=['pair','edge'], coords={'pair':echo_pairs, 'edge':np.arange(Ncons)})
            
            for (e_x,e_y) in echo_pairs_tuples:
                # Load Timeseries for first echo in echo-pair
                ...
                # Load Timeseries foe second echo in echo-pair
                ...
                # Covert loaded np.arrays to pd.dataframes and add ROI_name info as column names
                ...
                # Extract upper diagonal (e.g., remove redundant entries from connectivity matrix) and vectorize
            ...
            # Save average Fisher's transform across all echo pairs
            fc_all_sbjs_Z[f'across_echoes-{pipeline}-{censor_mode}'].loc[(sbj,ses),:] = aux_fc_xr.mean(axis=0).values
            # Apply inverse Fisher's transform to get average FC in the form of Pearson's Correlation
            fc_all_sbjs_R[f'across_echoes-{pipeline}-{censor_mode}'].loc[(sbj,ses),:] = np.tanh(aux_fc_xr.mean(axis=0).values)
...
mot_vs_fc = {}
scenarios =  ['e02-Basic-ALL','e02-GSasis-ALL', 'e02-Tedana-ALL',   'across_echoes-Basic-ALL',
              'e02-Basic-KILL','e02-GSasis-KILL', 'e02-Tedana-KILL','across_echoes-Basic-KILL']
...
pn.Column(pn.pane.Markdown('## FC (Correlation) vs. Motion (Mean Framewise Displacement) across pre-processing pipelines \n Top row shows results when no censoring is applied (e.g., ALL volumes) and bottom row when censoring is applied (e.g., KILL volumes)'), plot_grid).save('../../results/Corr_vs_Motion.html')
