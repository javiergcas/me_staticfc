...
def gen_scatter(dataset,data_fc,sbj,ses,pp,nordic,eep1,eep2,fc_metric, show_linear_fit=False, ax_lim=None, hexbin=False, title=None):
    ...
    fc_metric (str): FC metric to be used ('corr' for correlation, 'cov' for covariance)
...
def scan_summary_tab(sbj,ses,pp, nordic,fc_metric, 
    ...
    stats_mean_df.loc['Weighted Avg','pBOLD'] = data_qc[fc_metric,'pBOLD'].set_index(['Subject','Session','Pre-processing','m-NORDIC']).loc[sbj,ses,pp,nordic].values[0]
    
    TSNR_df                         = pd.DataFrame(columns=['Full Brain','Visual Cortex'],index=[pp])
    TSNR_df.loc[pp,'Full Brain']    = data_qc[fc_metric,'TSNR (Full Brain)'].set_index(['Subject','Session','Pre-processing','m-NORDIC']).loc[sbj,ses,pp,nordic].values[0]
    TSNR_df.loc[pp,'Visual Cortex'] = data_qc[fc_metric,'TSNR (Visual Cortex)'].set_index(['Subject','Session','Pre-processing','m-NORDIC']).loc[sbj,ses,pp,nordic].values[0]
    TSNR_df_ready_to_plot           = data_qc[fc_metric,'TSNR (Full Brain)'].set_index(['Subject','Session','m-NORDIC','Pre-processing']).loc[sbj,ses,nordic,:].reset_index()
    ...
    pBOLD_df.loc[pp,'pBOLD']    = data_qc[fc_metric,'pBOLD'].set_index(['Subject','Session','Pre-processing','m-NORDIC']).loc[sbj,ses,pp,nordic].values[0]
    pBOLD_df_ready_to_plot      = data_qc[fc_metric,'pBOLD'].set_index(['Subject','Session','m-NORDIC','Pre-processing']).loc[sbj,ses,nordic,:].reset_index()
    ...
    # Tedana Card
    # ===========
    ics_all_df    = data_qc['cov','#ICs (All)'].set_index(['Subject','Session','m-NORDIC','Pre-processing']).loc[sbj,ses,nordic,:].reset_index()
    ics_lbold_df  = data_qc['cov','#ICs (Likely BOLD)'].set_index(['Subject','Session','m-NORDIC','Pre-processing']).loc[sbj,ses,nordic,:].reset_index()
    ics_ulbold_df = data_qc['cov','#ICs (Unlikely BOLD)'].set_index(['Subject','Session','m-NORDIC','Pre-processing']).loc[sbj,ses,nordic,:].reset_index()
    ...
    if data_qc[('cov','Physio (resp)')] is not None:
        if (sbj,ses) in data_qc[('cov','Physio (resp)')].index:
            resp_plot = data_qc[('cov','Physio (resp)')].hvplot.scatter(x='Mean',y='St.Dev.', c='color', title='Card. Inter-peak Interval', aspect='square', hover_cols=['Subject','Run'], alpha=0.5, width=300, height=300) 
            resp_plot = resp_plot * hv.Points((data_qc[('cov','Physio (resp)')].loc[(sbj,ses)]['Mean'],data_qc[('cov','Physio (resp)')].loc[(sbj,ses)]['St.Dev.'])).opts(size=10, marker="o",line_color="black", line_width=2,fill_alpha=0.0)
            resp_plot = resp_plot.opts(shared_axes=False)
        if (sbj,ses) in data_qc[('cov','Physio (cardiac)')].index:
            card_plot = data_qc[('cov','Physio (cardiac)')].hvplot.scatter(x='Mean',y='St.Dev.', c='color', title='Resp. Inter-peak Interval', aspect='square', hover_cols=['Subject','Run'], alpha=0.5, width=300, height=300) 
            card_plot = card_plot * hv.Points((data_qc[('cov','Physio (cardiac)')].loc[(sbj,ses)]['Mean'],data_qc[('cov','Physio (cardiac)')].loc[(sbj,ses)]['St.Dev.'])).opts(size=10, marker="o",line_color="black", line_width=2,fill_alpha=0.0)
...
def fc_across_echoes_scatter_page(dataset,sbj,ses,pp, nordic,fc_metric, pairs_of_echo_pairs, 
    ...
    pp (str): pre-processing pipeline
    nordic (str): whether Nordic was used or not
    fc_metric (str): FC metric to be used ('cor' for correlation, 'cov' for covariance)
    ...
    for i in pairs_of_echo_pairs:
        ...
        title = 'pBOLD=%.2f' % (this_scatter_pBOLD)
...
def get_cov_heatmap(data,sbj,ses,pp_opts,nordic_opts,roi_info=None,clim=5, echo_pairs=echo_pairs):
    ...
    for nordic in nordic_opts.values():
        for pp in pp_opts.values():
            df = pd.DataFrame([data[(sbj, ses, pp, nordic, ep, 'cov')] for ep in echo_pairs], index=echo_pairs)
...
#     Generate scatter plot for two different FC matrices
#     """
#     if (sbj,ses,pp,nordic,eep1,'cov') not in data:
#         print('%s,%s,%s,%s,%s,cov not in data' % (sbj,ses,pp,nordic,eep1))
#         return pn.pane.Markdown('#Not Available')
...
#     data_df[eep1] = data[sbj,ses,pp,nordic,eep1,'cov']
#     data_df[eep2] = data[sbj,ses,pp,nordic,eep2,'cov']
#     # Compute limits for X and Y axis
#     if ax_lim is None:
#         lims = (data_df.quantile(0.01).min(),data_df.quantile(0.99).max())
#     else:
...
#     # Statistics Table 
#     stats_df      = qa_data.loc[sbj,ses,pp,nordic,'cov',:,:].to_dataframe(name='QC').reset_index().drop(['sbj','ses','pp','fc_metric'],axis=1).pivot(index='ee_vs_ee', columns='qc_metric', values='QC')
#     stats_mean_df = pd.DataFrame(stats_df.mean(),columns=['Avg']).T
...
def get_static_report(qa,fc_metric,qc_metric,x='Pre-processing',hue='m-NORDIC',show_stats=False, stat_test='t-test_paired',stat_annot_type='star', legend_location='best', remove_outliers_from_swarm=True, remove_outliers_from_statistics=False, dot_size=1, palette='Set2', show_points=False,session='all', label_font_size=12, pair_weights=None):
    ...
    hue (str): column name for the hue (default: 'm-NORDIC')
    ...
    if isinstance(qa, xr.DataArray):
        if pair_weights is None:
            ...
            df         = qa.weighted(pair_weights).mean(dim='ee_vs_ee').sel(fc_metric='cov', qc_metric='pBOLD').to_dataframe(name='cov').drop(['fc_metric','qc_metric'],axis=1).reset_index()
        df.columns = ['Subject','Session','Pre-processing','m-NORDIC',qc_metric]
