...
import numpy as np
import sys
from sfim_lib.cpm.cpm import mk_kfold_test_indices, mk_kfold_test_indices_subject_aware, split_train_test, get_train_test_data, select_features, build_model, apply_model, residualize
import pickle
...
def read_command_line():
    ...
    parser.add_argument('-l','--label',               type=str,   help="Label to differentiate outcomes",              required=True, dest='label')
...
def main():
    ...
    print('   * [%s] FC Dataframe Path            : %s' % (opts.label,opts.fc_path))
    assert osp.exists(opts.fc_path), '++ ERROR [main]: FC Dataframe not found.'
    if opts.label is not None:
       aux_out_dir = osp.join(opts.output_dir, opts.label)
    else:
       aux_out_dir = opts.output_dir
    print('   * [%s] Output Folder                : %s' % (opts.label,aux_out_dir), end=' | ')
    ...
    print("++ INFO [main]: Loading [%s] FC matrices from %s" % (opts.label, opts.fc_path))

    # Changed index_col to ['Subject']? Causes sfim module not found error

    fc_data         = pd.read_pickle(opts.fc_path)
    Nscans, n_edges = fc_data.shape
    fc_data.columns = range(n_edges) # Needed to ensure the proper type on the column ids
    print('++ INFO [main]: FC data loaded into memory [FC Label=%s, #Scans=%d, #Connections=%d]' % (opts.label,Nscans,n_edges))

    # 2. Load Behaviors table into memory
    behav_data = pd.read_pickle(opts.behav_path)    
    Nbehavs = behav_data.shape[1]
    print ('++ INFO [main]: Behaviors table loaded into memory [#Behaviors=%d]' % Nbehavs)
    if opts.verbose:
        print(' + Behavioral.index has %d entries [%s ...]' % (behav_data.shape[0],str(behav_data.index[0:3])))

    # 3. Load Motion Confound information (if needed)
    if opts.confounds:
       print ('++ INFO [main]: Loading confounds for residalization')
       confounds = pd.read_pickle(opts.confounds_path)
       assert 'Mean Motion (enorm)' in confounds.columns, "++ ERROR: Confound file does not contain expected column Mean Rel Motion"
    ...
    assert fc_data.index.equals(behav_data.index), "++ ERROR [main]:Index in FC dataFrame [%s] and behavior dataframe do not match." % opts.label
    if opts.confounds:
       assert fc_data.index.equals(confounds.index), "++ ERROR [main]: Index in FC dataframe [%s] and motion confound do not match." % opts.label
    ...
    # 7. Extract a list of scan IDs. Now relies on behav_data instead of fc_data
    scan_list = behav_data.index
    ...
    for fold in range(opts.k):
        ...
        if opts.confounds:
            ...
            tr_confounds = confounds.loc[tr_scans].infer_objects()
            tt_confounds = confounds.loc[tt_scans].infer_objects()
    ...
    output_file = osp.join(aux_out_dir,'cpm_{beh}_rep-{rep}.pkl'.format(rep=str(opts.repetition).zfill(5), beh=opts.behavior))
    ...
    for tail in ["pos","neg","glm"]:
        print('++ INFO [main]: R(obs,pred-%s,%s) = %.3f' % (tail,opts.label, behav_obs_pred[opts.behavior + " observed"].corr(behav_obs_pred[opts.behavior + " predicted (" + tail + ")"])))
